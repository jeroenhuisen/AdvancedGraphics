	...
	for (tinyobj::shape_t shape : shapes) {
		...
		for (size_t f = 0; f < shape.mesh.indices.size() / 3; f++) {
			...
			if (attrib.normals.size() > 0) {
				...
				vec3 normal1 = vec3(attrib.normals[3 * n0], attrib.normals[3 * n0 + 1], attrib.normals[3 * n0 + 2]);
				vec3 normal2 = vec3(attrib.normals[3 * n1], attrib.normals[3 * n1 + 1], attrib.normals[3 * n1 + 2]);
				vec3 normal3 = vec3(attrib.normals[3 * n2], attrib.normals[3 * n2 + 1], attrib.normals[3 * n2 + 2]);
				//std::cout << "x: " << x << " y: " << y << " z: " << z << std::endl;
				t = createTriangle(v1, v2, v3, normal1, normal2, normal3, *pink);
			}
			else {
				t = createTriangle(v1, v2, v3, *pink);
			}
...
/*
ImportObject* ObjectImporter::loadObjectImportObject(const std::string filename, int* objSize) {
	...
	*objSize = shapes.size();
	for (tinyobj::shape_t shape : shapes) {
		...
		for (size_t f = 0; f < shape.mesh.indices.size() / 3; f++) {
			...
			trianglePtr->v1 = { v1.x, v1.y, v1.z };
			trianglePtr->v2 = { v2.x, v2.y, v2.z };
			trianglePtr->v3 = { v3.x, v3.y, v3.z };
			//trianglePtr->centroid = (v1 + v2 + v3) / 3.0f;
			trianglePtr->color = { 1, 0, 1 };// pink;
			...
			if (attrib.normals.size() > 0) {
				...
				vec3 normal3 = vec3(attrib.normals[3 * n2], attrib.normals[3 * n2 + 1], attrib.normals[3 * n2 + 2]);
				//std::cout << "x: " << x << " y: " << y << " z: " << z << std::endl;
				vec3 dir = normalize(normal1 + normal2 + normal3);
				trianglePtr->direction = { dir.x, dir.y, dir.z };
